...
export const ExecutiveHome = () => {
    ...
    // Trend Card calculation
    const trendData = useMemo(() => {
        const last7Days = Array.from({length: 7}, (_, i) => {
             const d = subDays(selectedDate, 6 - i);
             const dStr = format(d, "yyyy-MM-dd");
             const done = ownVisits.filter(v => v.visit_date === dStr && v.status === 'done').length;
             return {
    ...
    const markDone = async (visit: Visit) => {
        try {
            const { error } = await supabase.from("visits").update({ status: "done" }).eq("id", visit.id);
            if (error) throw error;
            ...
            <div className="p-3 sm:p-5 space-y-6 sm:space-y-8 w-full max-w-[100vw] overflow-x-hidden">
                     ...
                     <div className="flex overflow-x-auto snap-x snap-mandatory gap-3 pb-4 -mx-4 sm:-mx-5 px-4 sm:px-5 sm:grid sm:grid-cols-3 sm:mx-0 sm:px-0 sm:pb-0 sm:overflow-visible no-scrollbar">
                              ...
                              <div className="space-y-2.5">
                                   {leaderboard.visits.map((exec, idx) => (
                                        <div key={exec.user_id} className="flex items-center justify-between cursor-pointer hover:bg-[#F5F5F7]/10 p-1 -mx-1 rounded transition-colors" onClick={() => setLeadPopup({ name: exec.full_name || 'Executive', visits: exec.visits, wosCount: exec.wosCount, wosWon: exec.wosWon, rankingLogic: 'Ranked by Total Visits Completed' })}>
                                             <span className={`text-xs truncate max-w-[60px] ${exec.user_id === user?.id ? 'text-[#F5F5F7] font-semibold' : 'text-[#A1A5AE]'}`}>
                                   ...
                                   {leaderboard.visits.length === 0 && <p className="text-xs text-center text-[#8E939D]">No data</p>}
                              </div>
                         </div>
                         ...
                         <div className="w-[85%] max-w-[280px] sm:w-auto shrink-0 snap-center bg-[#1A1D24] rounded-xl p-3 border border-[#F5F5F7]/5 flex flex-col justify-between">
                              ...
                              <div className="space-y-2.5">
                                   ...
                                   {leaderboard.wosCount.length === 0 && <p className="text-xs text-center text-[#8E939D]">No data</p>}
                              </div>
                         </div>
                              ...
                              <div className="space-y-2.5">
                                   {leaderboard.wosWon.map((exec, idx) => (
                                        <div key={exec.user_id} className="flex items-center justify-between cursor-pointer hover:bg-[#F5F5F7]/10 p-1 -mx-1 rounded transition-colors" onClick={() => setLeadPopup({ name: exec.full_name || 'Executive', visits: exec.visits, wosCount: exec.wosCount, wosWon: exec.wosWon, rankingLogic: 'Ranked by Verified WOS Value' })}>
                                             <span className={`text-xs truncate max-w-[55px] ${exec.user_id === user?.id ? 'text-[#F5F5F7] font-semibold' : 'text-[#A1A5AE]'}`}>
                                                  {exec.full_name?.split(' ')[0]}
                                             </span>
                                             <div className="flex items-center gap-1.5">
                                   ...
                                   {leaderboard.wosWon.length === 0 && <p className="text-xs text-center text-[#8E939D]">No data</p>}
                              </div>
                         </div>
                     </div>
                </div>
               ...
               {/* KPI RINGS - TOTAL */}
                ...
                <div className="bg-[#12141A] rounded-2xl p-4 sm:p-5 border border-[#F5F5F7]/5 shadow-sm mt-6 sm:mt-8">
                    ...
                    <div className="h-28 flex items-end justify-between gap-2.5 border-b border-[#F5F5F7]/5 pb-2 relative">
                        <div className="absolute top-1/2 left-0 right-0 border-t border-dashed border-[#F5F5F7]/5" />
                        {trendData.map((data, i) => (
                            <div key={i} className="w-full bg-[#A6192E]/5 rounded-t-sm relative group overflow-hidden">
                                <motion.div
                                    initial={{ height: 0 }}
                                    animate={{ height: `${(data.val / Math.max(10, ...trendData.map(d=>d.val))) * 100}%` }}
                                    transition={{ duration: 1, delay: i * 0.1 }}
                                    className={`absolute bottom-0 w-full rounded-t-sm ${i === 6 ? 'bg-[#C21833]' : 'bg-[#A6192E]/40'}`}
                                />
                            </div>
                        ))}
                    </div>
                    <div className="flex justify-between mt-3 text-[10px] text-[#8E939D] font-semibold uppercase tracking-widest">
                        {trendData.map((d, i) => <span key={i} className={i === 6 ? "text-[#F5F5F7]" : ""}>{d.day}</span>)}
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-3 mt-8">
                    ...
                    <StackedCard icon={<Clock className="h-4 w-4 text-[#D1D5DB]" />} title="Pending Follow-ups" subtitle={`${dailyKpis.pendingFollowups} due today`} badge={dailyKpis.pendingFollowups} badgeColor="bg-[#C21833]" />
                    <StackedCard icon={<AlertCircle className="h-4 w-4 text-[#D1D5DB]" />} title="Overdue TAT Alerts" subtitle="Requires immediate action" badge={dailyKpis.overdueCount} badgeColor="bg-[#C21833]" />
                ...
                <div className="mt-6 sm:mt-8">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 mb-4">
                        ...
                        {isToday(selectedDate) && !todayAttendance && (
                             <button 
                                ...
                                className="bg-[#1A1D24] text-[#F5F5F7] px-4 py-1.5 rounded-full text-xs font-semibold border border-[#2E7D32]/50 hover:bg-[#2E7D32]/20 transition-all flex items-center gap-2 disabled:opacity-50 w-fit shadow-md"
                             >
                             ...
                             <div className="flex items-center gap-3">
                                 ...
                                 {!endDayRecord ? (
                                     <button 
                                        ...
                                        className="bg-[#1A1D24] text-[#A1A5AE] hover:text-[#F5F5F7] px-4 py-1.5 rounded-full text-xs font-semibold border border-[#F5F5F7]/10 hover:border-[#F5F5F7]/30 transition-all flex items-center gap-2 disabled:opacity-50"
                                     >
                                        <History className="h-3.5 w-3.5" />
                                        {isEndingDay ? 'Ending...' : 'End Day'}
                                     </button>
                                 ) : (
                                     <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#1A1D24] border border-[#F5F5F7]/10 w-fit shadow-sm">
                                         <CheckCircle2 className="w-3.5 h-3.5 text-[#A1A5AE]" />
                                         <span className="text-xs font-semibold text-[#A1A5AE]">Day Ended</span>
                                     </div>
                                 )}
                             </div>
                        )}
                    ...
                    <div className="space-y-3">
                        {dailyKpis.todayVisits.length === 0 ? (
                            <div className="text-center py-12 px-4 bg-[#12141A] rounded-2xl border border-[#F5F5F7]/5 shadow-sm">
                                <div className="w-16 h-16 bg-[#1A1D24] rounded-full flex items-center justify-center mx-auto mb-4 border border-[#F5F5F7]/5 shadow-inner">
                                    <CalendarCheck className="h-6 w-6 text-[#8E939D]" />
                                </div>
                                <p className="text-[#F5F5F7] font-semibold text-lg mb-1">Your day is clear</p>
                                <p className="text-sm text-[#A1A5AE] mb-5">Take a moment to plan ahead or scout new leads.</p>
                                <button onClick={() => navigate("/visits")} className="text-[#F5F5F7] font-semibold text-sm bg-[#1A1D24] hover:bg-[#F5F5F7]/10 px-5 py-2.5 rounded-xl border border-[#F5F5F7]/5 transition-colors">Plan a Visit</button>
                            </div>
                        ) : (
                ...
                <div className="mt-8">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="text-lg font-bold text-[#F5F5F7] tracking-tight">My Conveyance</h3>
                        <span className="text-[10px] text-[#8E939D] uppercase tracking-widest font-semibold">
                            {isToday(selectedDate) ? "Today" : format(selectedDate, "dd MMM")}
                        </span>
                    </div>

                    {todayConveyance.length === 0 ? (
                        <div className="bg-[#12141A] rounded-2xl border border-[#F5F5F7]/5 p-5 flex flex-col items-center justify-center text-center gap-2">
                            <Navigation className="h-8 w-8 text-[#3A3F4B] mb-1" />
                            <p className="text-[#F5F5F7] font-semibold text-sm">No trips recorded yet</p>
                            <p className="text-xs text-[#8E939D]">Start Day and mark visits done to track conveyance.</p>
                        </div>
                    ) : (
                        <div className="space-y-2">
                            ...
                            <div className="bg-[#1A1D24] rounded-2xl p-3 sm:p-4 border border-[#F5F5F7]/5 flex flex-wrap sm:flex-nowrap items-center justify-between gap-3 mb-3 shadow-sm">
                                <div className="flex items-center gap-2">
                                    ...
                                    <div>
                                        <p className="text-xs text-[#8E939D] font-semibold uppercase tracking-widest">Total Distance</p>
                                        <p className="text-lg font-bold text-[#F5F5F7] font-mono">{todayTotalKm.toFixed(1)} <span className="text-sm text-[#8E939D] font-sans">km</span></p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <p className="text-xs text-[#8E939D] font-semibold uppercase tracking-widest">Total Earned</p>
                            ...
                            {todayConveyance.map((trip, i) => (
                                <div key={trip.id} className="bg-[#12141A] rounded-xl border border-[#F5F5F7]/5 px-4 py-3 flex items-center gap-3">
                                    <div className="flex flex-col items-center gap-0.5 shrink-0">
                                        <div className="w-2 h-2 rounded-full bg-[#8E939D]" />
                                        <div className="w-px h-4 bg-[#F5F5F7]/10" />
                                        <div className="w-2 h-2 rounded-full bg-[#2E7D32]" />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className="text-xs text-[#A1A5AE] truncate"><span className="text-[#8E939D]">{i === 0 ? 'Start' : 'From'}:</span> {trip.from_location_name}</p>
                                        <p className="text-xs text-[#F5F5F7] truncate font-medium"><span className="text-[#8E939D]">To:</span> {trip.to_location_name}</p>
                                    </div>
                                    <div className="text-right shrink-0">
                                        <p className="text-xs font-bold text-[#F5F5F7] font-mono">{trip.distance_km} km</p>
        ...
        <div className="flex flex-col items-center justify-center cursor-pointer group hover:scale-105 transition-transform" onClick={onClick}>
            <div className="relative flex items-center justify-center mb-3">
                ...
                <div className="absolute flex flex-col items-center justify-center text-center w-full">
                    <span className="text-lg font-bold text-[#F5F5F7] tracking-tighter leading-none">{displayValue}</span>
                </div>
            </div>
            <div className="text-center">
                <p className="text-[11px] font-semibold text-[#D1D5DB] leading-tight mb-0.5">{label}</p>
                <p className="text-[9px] font-medium text-[#8E939D] uppercase tracking-wider leading-none max-w-[80px] text-center truncate">{sublabel}</p>
            </div>
        </div>
    );
};
...
const StackedCard = ({ icon, title, subtitle, badge, badgeColor = "bg-[#C21833]", onClick }: StackedCardProps) => (
    <div onClick={onClick} className={`bg-[#12141A] rounded-2xl p-4 flex items-center justify-between border border-[#F5F5F7]/5 shadow-sm group ${onClick ? 'cursor-pointer hover:bg-[#1A1D24] transition-colors' : ''}`}>
        <div className="flex items-center gap-4">
            <div className="w-10 h-10 rounded-xl bg-[#1A1D24] border border-[#F5F5F7]/5 shadow-inner flex items-center justify-center">
                {icon}
            </div>
            <div>
                <p className="text-sm font-semibold text-[#F5F5F7] mb-0.5">{title}</p>
                <p className="text-[11px] text-[#A1A5AE] font-medium">{subtitle}</p>
            </div>
        </div>
        <div className="flex items-center gap-3">
            {badge && badge > 0 ? (
                <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold text-[#F5F5F7] ${badgeColor} shadow-sm`}>
                    {badge}
                </span>
            ) : null}
            <ChevronRightIcon className="h-4 w-4 text-[#8E939D] group-hover:text-[#F5F5F7] transition-colors" />
        </div>
    </div>
);
...
const VisitCard = ({ visit, onMarkDone, onCancel, index, navigate }: VisitCardProps) => {
    ...
    return (
        <motion.div
            ...
            className="relative rounded-2xl overflow-hidden bg-[#12141A] touch-pan-y shadow-sm"
        >
            {/* Background Actions */}
            <div className={`absolute inset-0 flex items-center justify-between px-6 transition-colors duration-200 ${action === "done" ? "bg-[#276749]" : action === "cancel" ? "bg-[#C21833]" : "bg-[#1A1D24]"}`}>
                <span className={`text-[#F5F5F7] font-bold flex items-center gap-2 transition-opacity ${action === "done" ? 'opacity-100' : 'opacity-0'}`}><CheckCircle2 className="h-5 w-5" /> DONE</span>
                <span className={`text-[#F5F5F7] font-bold flex items-center gap-2 transition-opacity ${action === "cancel" ? 'opacity-100' : 'opacity-0'}`}>CANCEL <X className="h-5 w-5" /></span>
            </div>
            ...
            <motion.div
                ...
                animate={controls}
                className="relative bg-[#12141A] p-5 rounded-2xl border border-[#F5F5F7]/5 z-10 w-full"
            >
                <div className="flex justify-between items-start mb-2.5">
                    <p className="text-[#A1A5AE] text-[10px] font-semibold tracking-widest uppercase">{format(parseISO(visit.visit_date), "hh:mm a")} <span className="mx-1 text-[#6B7280]">•</span> <span className={`text-${isDone ? 'emerald' : 'blue'}-400`}>{visit.status.replace("_", " ")}</span></p>
                    <div className={`w-2 h-2 rounded-full ${statusColor} shadow-sm`} />
                </div>
                <h3 className="text-[#F5F5F7] font-semibold text-lg leading-tight mb-1 truncate">{visit.clients?.name || visit.partners?.name || "Meeting"}</h3>
                <p className="text-[#A1A5AE] text-xs font-medium mb-4">{visit.purpose_masters?.purpose_name || visit.purpose || "Follow-up"}</p>

                <div className="flex items-center gap-2 text-xs text-[#D1D5DB] mb-5 bg-[#1A1D24] p-2.5 rounded-xl border border-[#F5F5F7]/5 shadow-inner">
                    <MapPin className="h-3.5 w-3.5 text-[#A6192E] shrink-0" />
                    <span className="truncate leading-none pt-0.5">{visit.address || "Location pending"}</span>
                </div>

                <div className="flex gap-2.5">
                    <button className="flex-1 bg-[#1A1D24] hover:bg-[#F5F5F7]/10 text-[#F5F5F7] rounded-xl py-2.5 text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors border border-[#F5F5F7]/5">
                        <Navigation className="h-3.5 w-3.5 text-[#A1A5AE]" /> Navigate
                    </button>
                    {!isDone && (
                        <button onClick={() => navigate("/verification")} className="flex-1 bg-[#C21833] hover:bg-[#A6192E] text-[#F5F5F7] rounded-xl py-2.5 text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors shadow-sm">
                ...
                {!isDone && (
                    <div className="mt-4 flex items-center justify-center gap-2">
                        ...
                        <span className="text-[9px] font-semibold text-[#8E939D] uppercase tracking-widest">Swipe to mark done</span>
                        <ChevronRightIcon className="h-3 w-3 text-[#6B7280] animate-pulse" />
                    </div>
                )}
            </motion.div>
            ...
            <AnimatePresence>
                {open && (
                    <motion.div
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="fixed inset-0 bg-[#0A0B0E]/80 z-40 backdrop-blur-sm"
                ...
                <AnimatePresence>
                    {open && (
                        ...
                        >
                            ...
                            <ActionButton label="Add Partner" icon={<Building2 className="h-4 w-4" />} color="bg-[#1A1D24] border border-[#F5F5F7]/10" onClick={() => { setOpen(false); navigate("/partners"); }} />
                            <ActionButton label="Add Client" icon={<Users className="h-4 w-4" />} color="bg-[#1A1D24] border border-[#F5F5F7]/10" onClick={() => { setOpen(false); navigate("/clients"); }} />
                            <ActionButton label="Plan Visit" icon={<CalendarCheck className="h-4 w-4" />} color="bg-[#C21833]" onClick={() => { setOpen(false); navigate("/visits"); }} />
                        </motion.div>
                    )}
                </AnimatePresence>
                <motion.button
                    whileTap={{ scale: 0.9 }}
                    onClick={toggle}
                    className="h-14 w-14 rounded-full bg-[#C21833] text-[#F5F5F7] shadow-lg shadow-black/40 flex items-center justify-center transition-all hover:bg-[#A6192E] active:scale-95"
                >
...
const ActionButton = ({ label, icon, color, onClick }: ActionButtonProps) => (
    <div className="flex items-center gap-3">
        <span className="text-[#F5F5F7] text-[11px] font-semibold tracking-wider uppercase bg-[#1A1D24] px-3.5 py-2 rounded-xl shadow-md border border-[#F5F5F7]/5">{label}</span>
        <button onClick={onClick} className={`h-10 w-10 rounded-full ${color} text-[#F5F5F7] shadow-lg flex items-center justify-center`}>
            {icon}
        </button>
    </div>
);
