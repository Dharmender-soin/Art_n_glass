-- ─────────────────────────────────────────────────────────────
-- MIGRATION: Fix Notification HTTP Headers for Edge Functions
-- DATE: 2026-06-30
-- ─────────────────────────────────────────────────────────────

-- 1. Fix process_scheduled_notifications function to include Authorization header
...
BEGIN
  ...
  LOOP
    ...
    -- Call Edge Function via pg_net with proper Authorization header
    BEGIN
      SELECT net.http_post(
        url := 'https://khuqshdbpmuolyarhuud.supabase.co/functions/v1/send-push-notification',
        headers := '{"Content-Type": "application/json", "apikey": "sb_publishable_Rs8MWYZC0DOgeqmcTc9pGg_VNCOqbCz", "Authorization": "Bearer sb_secret_sli3qU6nC-9_JU1E_T84og_IY1W8em4"}'::jsonb,
        body := payload
      ) INTO req_id;
...
-- 2. Fix Automated Triggers with proper Authorization header
...
-- 3. Reschedule cron tasks to use updated headers
