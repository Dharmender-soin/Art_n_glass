...
interface RawWOS {
  ...
  verified_at: string | null; quantity: number | null;
  description: string | null; created_by: string;
  clients: { name: string; address: string | null; mobile: string; partners: { name: string } | null } | null;
...
interface PivotClient {
  ...
  client_mobile: string; partner_name: string | null; // builder/architect
  wos: Record<string, WOSRecord>;
  partners: string[]; // work type sub_work pills for WOSs column
}
...
const Hierarchy = () => {
  ...
  const { data: rawWOS=[], isLoading } = useQuery({ queryKey:["wos-h3"], enabled:canAccess,
    queryFn: async()=>{
      const{data,error}=await supabase.from("work_scope_items")
        .select(`id,client_id,work_type_id,work_status,created_at,submitted_at,verified_at,quantity,description,created_by,clients(name,address,mobile,partners(name)),master_work_types(type_of_work,sub_work)`)
  ...
  const pivotData = useMemo(():PivotExecutive[]=>{
    const em=new Map<string,PivotExecutive>();
    // Build wtId -> sub_work lookup
    ...
    rawWOS.forEach(r=>{
      ...
      if(!cl){
        ...
        cl={client_id:r.client_id,client_name:r.clients.name,client_address:r.clients.address||"—",client_mobile:r.clients.mobile,partner_name:builderName,wos:{},partners:[]};
      ...
      // collect unique WOS sub_work names for WOSs column
    ...
    return res;
  },[rawWOS,profileMap,showroomMap,fExec,fStatus,fShowroom,fSearch,allWorkTypes]);
  ...
  const totalRows = pivotData.reduce((s,e)=>s+e.clients.length,0);
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-20 text-sm">
      ...
      <div className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-5 py-3">
        <div className="flex items-center justify-between">
          ...
          {/* Right: export */}
          <button className="flex items-center gap-1.5 text-xs font-semibold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-200 dark:hover:bg-slate-700 px-3 py-1.5 rounded-lg transition-all">
      ...
      <div className="px-5 pb-4">
        {!isLoading && (
          <p className="text-[11px] font-semibold text-slate-400 dark:text-slate-500 mb-2">
            {totalRows} records · {pivotData.length} executives
          </p>
        )}
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center py-16 gap-3 text-center">
            <GitBranch className="h-10 w-10 text-slate-200 dark:text-slate-700"/>
            <p className="text-sm font-bold text-slate-500">{rawWOS.length===0?"No WOS data yet":"No results for current filters"}</p>
            ...
            <div className="overflow-auto" style={{maxHeight:"calc(100vh - 340px)"}}>
              <table className="w-full text-xs border-collapse min-w-max">
                ...
                <thead className="sticky top-0 z-10">
                  <tr>
                    {/* Fixed cols header row 1 */}
                    <th rowSpan={2} className="bg-slate-900 dark:bg-slate-950 text-slate-300 border-b-2 border-r border-slate-700 px-3 py-2.5 text-left font-bold uppercase tracking-wider text-[10px] min-w-[120px] sticky left-0 z-20 align-bottom">
                      Executive
                    </th>
                    <th rowSpan={2} className="bg-slate-800 dark:bg-slate-800/80 text-slate-400 border-b-2 border-r border-slate-700 px-3 py-2.5 text-left font-bold uppercase tracking-wider text-[10px] min-w-[90px] align-bottom">
                      Partner
                    </th>
                    <th rowSpan={2} className="bg-slate-800 dark:bg-slate-900 text-slate-400 border-b-2 border-r border-slate-700 px-3 py-2.5 text-left font-bold uppercase tracking-wider text-[10px] min-w-[130px] align-bottom">
                      Client Name
                    </th>
                    <th rowSpan={2} className="bg-slate-800 dark:bg-slate-900 text-slate-400 border-b-2 border-r border-slate-700 px-3 py-2.5 text-left font-bold uppercase tracking-wider text-[10px] min-w-[120px] align-bottom">
                      Address
                    </th>
                    <th rowSpan={2} className="bg-slate-800 dark:bg-slate-900 text-slate-400 border-b-2 border-r border-slate-700 px-3 py-2.5 text-center font-bold uppercase tracking-wider text-[10px] min-w-[95px] align-bottom">
                      Mobile
                    </th>
                    {/* Work type group headers */}
                    ...
                    {/* WOSs last column */}
                    <th rowSpan={2} className="bg-slate-700 dark:bg-slate-700 text-slate-300 border-b-2 border-r border-slate-600 px-3 py-2.5 text-center font-bold uppercase tracking-wider text-[10px] min-w-[110px] align-bottom">
                      WOSs
                    </th>
                  </tr>
                  <tr>
                  ...
                  {pivotData.map((exec, ei) =>
                    exec.clients.map((client, ci) => (
                      <tr key={`${exec.executive_id}-${client.client_id}`}
                        ...
                        {/* Partner cell — builder/architect */}
                        ...
                        {colIds.map(wtId=>{
                          ...
                          return (
                            <td key={wtId} className="px-1.5 py-1.5 border-r border-slate-100 dark:border-slate-800">
                              {rec
                                ? <Badge rec={rec} onClick={()=>{ setSelectedCell(rec); setUpdateStatus(rec.work_status); }}/>
                        ...
                        {/* WOSs — last column: sub_work pills */}
